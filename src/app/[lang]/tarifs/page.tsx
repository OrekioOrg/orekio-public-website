...
import { stripePaymentLinks } from "@/stripe";
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-3">
        {dict.plans.map((plan) => {
          // "Découverte" (free trial) and "Cabinet" (custom quote) aren't
          // card payments — they route to contact. Only "Praticien" checks
          // out through Stripe, via a Payment Link configured in
          // src/stripe.ts (this site has no server to create Checkout
          // Sessions, so a Payment Link is the static-friendly equivalent).
          const checkoutHref = stripePaymentLinks[plan.id];
          const buttonClassName =
            "mt-8 inline-block rounded-lg px-6 py-3 text-center text-[15px] font-medium transition-colors";
          ...
          return (
            ...
            >
              ...
              {plan.id !== "praticien" ? (
                <Link
                  href={`/${lang}/contact`}
                  className={`${buttonClassName} border border-outline bg-surface text-on-surface-strong hover:bg-primary-container`}
                >
                  {dict.quoteButton}
                </Link>
              ) : checkoutHref ? (
                <a
                  href={checkoutHref}
                  className={`${buttonClassName} bg-primary text-on-primary hover:opacity-90`}
                >
                ...
                <button
                  ...
                  className={`${buttonClassName} cursor-not-allowed bg-primary/40 text-on-primary`}
                >
                  {dict.subscribeButtonComingSoon}
                </button>
              )}
            </div>
          );
