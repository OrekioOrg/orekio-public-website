...
function MailIcon() {
  return (
    <svg viewBox="0 0 24 24" className={ICON} aria-hidden="true">
      <path d="M1.5 8.67v8.58a3 3 0 003 3h15a3 3 0 003-3V8.67l-8.928 5.493a3 3 0 01-3.144 0zM22.5 6.908V6.75a3 3 0 00-3-3h-15a3 3 0 00-3 3v.158l9.714 5.978a1.5 1.5 0 001.572 0z" />
    </svg>
  );
}
function LinkIcon() {
...
export function ShareButtons({ url, title, labels }: Props) {
  ...
  const blueskyHref = `https://bsky.app/intent/compose?text=${enc(`${title} ${url}`)}`;
  const mailHref = `mailto:?subject=${enc(labels.shareEmailSubject + title)}&body=${enc(url)}`;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <a
        ...
        rel="noopener noreferrer"
        className={btn}
        aria-label={labels.shareBluesky}
        title={labels.shareBluesky}
      >
        <BlueskyIcon />
      </a>
      <a
